...
1. Setup `.nalej-component.json` file with version, application list, image list and tools (See [Set variables](#set-variables)).
...
4. Setup dependencies (See [Configure dependencies](#configure-dependencies)).
5. Ask DevOps team for final manual steps in Azure/Github
...
* `.nalej-component.json`: This file describes the component. It contains the language used, current version of the repo, application list, image list and additional tools required.
...
Inside the `ci` folder is the main pipeline definition file for the CI called `azure-pipelines.main.yaml`. You do not need to change anything inside the file. Pipeline yaml is ready to get automatic feedback from every change you push to the repository.

## Set variables

Below there is an example of .nalej-component.json
...
Any developed solution will be associated with a version. The value of the version must be indicated in the `version` variable.

In order to know what applications have to be generated, the list of applications have to be set. This is a comma separated list. The name of the apps must be the same of the folders under the cmd. This is done modifying the `application_list` variable in the `.nalej-component.json` file.

For the current example, example-app and other-app.

`image_list` will match the list of Docker images required for your component.
...
generated. Images are tagged using the version value set in the version variable inside `.nalej-component.json`.
...
We use [SemVer](http://semver.org/) for versioning. For the versions available, see the [tags on this repository](https://github.com/nalej/golang-template/tags). 

## Authors

See also the list of [contributors](https://github.com/nalej/golang-template/contributors) who participated in this project.
